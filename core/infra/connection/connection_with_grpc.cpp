...
See the Li
#include <string>cense for the specific language governing permissions and
...
std::function<RecieverConfirmation(const std::string&)> sign = [](
    const std::string& hash) {
  RecieverConfirmation confirm;
  Signature signature;
  signature.set_publickey(::peer::myself::getPublicKey());
  signature.set_signature(signature::sign(hash, ::peer::myself::getPublicKey(),
...
std::function<bool(const RecieverConfirmation&)> valid =
    [](const RecieverConfirmation& c) {
      return signature::verify(c.signature().signature(), c.hash(),
                               c.signature().publickey());
    };
...
namespace Verify {
std::vector<
    std::function<void(const std::string& from, ConsensusEvent& message)> >
    receivers;
};
namespace Torii {
std::vector<std::function<void(const std::string& from, Transaction& message)> >
    receivers;
}
};
namespace Izanami {
...
namespace find {
std::vector<std::function<void(const std::string& from, Query& message)> >
    receivers;
};
namespace fetch {
std::vector<std::function<void(const std::string& from, Query& message)> >
...
std::vector<
    std::function<void(const std::string& from, DiscoverRequest& message)> >
    receivers;
}
};
 ...
 public:
  ...
  Response Verify(const ConsensusEvent& consensusEvent) {
    ...
    logger::info("connection") << "size: "
                               << consensusEvent.eventsignatures_size();
    logger::info("connection") << "name: "
                               << consensusEvent.transaction().asset().name();
    ...
    if (status.ok()) {
      ...
      return {response.value(),
              valid(response.confirm()) ? RESPONSE_OK : RESPONSE_INVALID_SIG};
    } else {
...
class SumeragiConnectionServiceImpl final : public Sumeragi::Service {
 public:
  Status Verify(ServerContext* context, const ConsensusEvent* pevent,
  ...
  Status Torii(ServerContext* context, const Transaction* transaction,
               StatusResponse* response) override {
    RecieverConfirmation confirm;
    auto dummy = "";
    Transaction tx;
  ...
  Status Kagami(ServerContext* context, const Query* query,
                StatusResponse* response) override {
...
class IzanamiConnectionServiceImpl final : public Izanami::Service {
 public:
  Status Izanagi(ServerContext* context, const TransactionResponse* txResponse,
                 StatusResponse* response) override {
...
class TransactionRepositoryServiceImpl final
    : public TransactionRepository::Service {
 public:
  Status find(ServerContext* context, const Query* query,
  ...
  Status fetch(ServerContext* context, const Query* query,
               TransactionResponse* response) override {
    Query q;
    q.CopyFrom(*query);
    auto dummy = "";
  ...
  Status fetchStream(ServerContext* context, ServerReader<Transaction>* reader,
                     StatusResponse* response) override {
    Query q;
...
class AssetRepositoryServiceImpl final : public AssetRepository::Service {
 public:
  Status find(ServerContext* context, const Query* query,
              AssetResponse* response) override {
    std::string name = "default";
    Query q;
    q.CopyFrom(*query);
    ...
    if (q.type() == "asset") {
      response->mutable_asset()->CopyFrom(
          repository::asset::find(sender, name));
      logger::info("connection") << "-AssetRepositoryService: "
                                 << response->asset().DebugString();
    } else if (q.type() == "account") {
      response->mutable_account()->CopyFrom(repository::account::find(sender));
      logger::info("connection") << "-AccountRepositoryService: "
                                 << response->account().DebugString();
    }
    response->set_message("OK");
    return Status::OK;
  }
...
class HostDiscoverServiceImpl final : public HostDiscover::Service {
  Status getHostInfo(ServerContext* context, const DiscoverRequest* query,
                     Peer* response) override {
    DiscoverRequest request;
    request.CopyFrom(*query);
    logger::info("connection") << "HostDiscoverService: "
                               << request.DebugString();

    if (request.message() == "discovery") {
      response->set_publickey(
...
bool receive(
    const std::function<void(const std::string&, ConsensusEvent&)>& callback) {
...
bool send(const std::string& ip, const ConsensusEvent& event) {
  ...
  if (find(receiver_ips.begin(), receiver_ips.end(), ip) !=
      receiver_ips.end()) {
...
namespace Sumeragi {
bool send(const std::string& ip, const Transaction& transaction) {
  auto receiver_ips = ::peer::service::getIpList();
  if (find(receiver_ips.begin(), receiver_ips.end(), ip) !=
      receiver_ips.end()) {
    SumeragiConnectionClient client(grpc::CreateChannel(
...
namespace Izanami {
bool send(const std::string& ip, const TransactionResponse& txResponse) {
  IzanamiConnectionClient client(grpc::CreateChannel(
      ip + ":" + std::to_string(config::IrohaConfigManager::getInstance()
                                    .getGrpcPortNumber(50051)),
      grpc::InsecureChannelCredentials()));
...
namespace Izanagi {
bool receive(const std::function<void(const std::string&,
...
namespace find {
bool receive(const std::function<void(const std::string&, Query&)>& callback) {
  receivers.push_back(callback);
  return true;
}
};
...
Peer send(const std::string& ip, const DiscoverRequest& message) {
  HostDiscoverConnectionClient client(grpc::CreateChannel(
      ip + ":" + std::to_string(config::IrohaConfigManager::getInstance()
                                    .getGrpcPortNumber(50051)),
      grpc::InsecureChannelCredentials()));
...
bool receive(
    const std::function<void(const std::string&, DiscoverRequest&)>& callback) {
  receivers.push_back(callback);
  return true;
}
}
