...
#ifndef TOOLS_ISSUE_TRANSACTION_TRANSFER_HPP
#define TOOLS_ISSUE_TRANSACTION_TRANSFER_HPP
    ...
    namespace transfer {
      namespace account {
        void issue_transaction(std::vector<std::string> &argv) {
          try {
            auto
                tx = txbuilder::TransactionBuilder<type_signatures::Transfer<type_signatures::Account >>()
...
//TODO                                txbuilder::createAccount( argv[0], argv[1], txbuilder::createTrust( stod(argv[2]), argv[3]=="true" ) )
      ...
      namespace asset {
        void issue_transaction(std::vector<std::string> &argv) {
          try {
            auto
                tx = txbuilder::TransactionBuilder<type_signatures::Transfer<type_signatures::Asset >>()
                .setSenderPublicKey(::peer::myself::getPublicKey())
//TODO                                txbuilder::createAsset( argv[0], argv[1], txbuilder::createTrust( stod(argv[2]), argv[3]=="true" ) )
      ...
      namespace peer {
        void issue_transaction(std::vector<std::string> &argv) {
          try {
            auto
                tx = txbuilder::TransactionBuilder<type_signatures::Transfer<type_signatures::Peer >>()
                .setSenderPublicKey(::peer::myself::getPublicKey())
                .setPeer(
                    txbuilder::createPeer(argv.at(0), argv.at(1),
                                          txbuilder::createTrust(stod(argv.at(2)), argv.at(3) == "true"))
                )
                .build();
            connection::iroha::PeerService::Sumeragi::send(
                ::peer::myself::getIp(),
      ...
      namespace domain {
        void issue_transaction(std::vector<std::string> &argv) {
          try {
            auto
                tx = txbuilder::TransactionBuilder<type_signatures::Transfer<type_signatures::Domain >>()
                .setSenderPublicKey(::peer::myself::getPublicKey())
                .setDomain(
...
#endif // TOOLS_ISSUE_TRANSACTION_TRANSFER_HPP