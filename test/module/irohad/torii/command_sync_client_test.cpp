 ...
 * @when different methods of the object is called
 * @then the desired behavior achieved
 */
TEST_F(SyncClient, CommandClient) {
  ...
  EXPECT_CALL(*server, Status(_, _, _)).WillOnce(Return(grpc::Status()));
...
TEST_F(SyncClient, StatusStream) {
  ...
  resp.set_tx_hash(std::string(32, '1'));
