...
TEST_F(SimulatorTest, FailWhenProposalCannotBeCreated) {
  // proposal with height 2 => height 2 block present => no validated proposal
  auto proposal = makeProposal(2);

  auto block = makeBlock(proposal.height() - 1);
  ...
  EXPECT_CALL(*validator, validate(_, _)).WillOnce(Return(VerifiedProposalAndErrors{nullptr, {}}));
