...
class SumeragiServiceImpl : public ::protocol::Sumeragi::Service {
  ...
  virtual ::grpc::Status Torii(
    ...
    std::string pkstr = std::string(
      root->creator()->pubkey()->begin(),
      root->creator()->pubkey()->end());

    Log("verify only pubkey... '" + pkstr + "'");
    // ASSERT_STREQ(pkstr.c_str(), "hoge"); cannot use in server
    assert(pkstr == "hoge");
...
TEST_F(grpc_test, Sumeragi) {
  ...
  auto txbuf = gen_flatbuffer::CreateTx();
  ...
  ASSERT_EQ(reply->code(), protocol::RPCResponseCode::OK);
