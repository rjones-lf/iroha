...
class TransferAssetInterDomainTest : public TxPipelineIntegrationTestFixture {
 public:
  void SetUp() override {
    ...
    iroha::CryptoProviderImpl<>(adminKeypair).sign(genesis_transaction);
...
TEST_F(TransferAssetInterDomainTest, TransferAssetInterDomainTest) {
  ...
  iroha::CryptoProviderImpl<>(nbaKeypair).sign(transaction1);
  ...
  iroha::CryptoProviderImpl<>(nbaKeypair).sign(transaction2);
  ...
  iroha::CryptoProviderImpl<>(ivanKeypair).sign(transaction3);
