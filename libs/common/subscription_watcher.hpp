...
template <typename KeyType, typename ObservableTriggerType>
class SubscriptionWatcher {
 public:
  ...
  void addSubscription(const KeyType &key, rxcpp::subscription &subscription) {
    subscriptions_[key] = SubscriptionAndCounter{subscription, 0};
  }
 ...
 private:
  ...
  std::unordered_map<KeyType, SubscriptionAndCounter> subscriptions_;
  ...
  void incrementCounters() {
    std::lock_guard<std::mutex> lock{this->mutex_};
    std::for_each(std::begin(subscriptions_),
                  std::end(subscriptions_),
                  [](auto &entry) { entry.second.counter++; });
